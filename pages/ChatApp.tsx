...
// --- Sub-component: Note Detail Page (Replaces Modal) ---
...
const NoteDetailPage: React.FC<NoteDetailPageProps> = ({ note, onBack, onUpdateNote, onDeleteNote }) => {
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 dark:bg-slate-950">
                  {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-60">
                        <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400 rounded-2xl flex items-center justify-center mb-4">
                            <span className="text-3xl">👋</span>
                        </div>
                      <h3 className="text-xl font-bold text-slate-700 dark:text-slate-200 mb-2">Halo{user ? `, ${user.name.split(' ')[0]}` : ''}! Ada yang bisa dibantu?</h3>
                      <p className="text-slate-500 dark:text-slate-400 max-w-sm">
                        Mulai percakapan baru. Gunakan mode <strong>Q&A Interaktif</strong> untuk menguji pengetahuanmu!
                      </p>
                    </div>
                  ) : (
                    <div className="max-w-3xl mx-auto">
